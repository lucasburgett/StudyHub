...
def _generic_title(title: str | None) -> bool:
    if not title:
        return True
    rest = clean_lecture_title(title)
...
def rebuild_lectures(conn: sqlite3.Connection, course_id: int) -> int:
    ...
    for r in conn.execute(
        ...
        if n is not None and lec.number is None and n not in by_number:
            lec.number = n
            by_number[n] = lec
        if not lec.title and not _generic_title(r["title"]):
            lec.title = clean_lecture_title(r["title"])
