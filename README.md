...
| **GoodNotes** | Turn on **Settings → Automatic Backup**, choose Google Drive and **PDF**. Set `GOODNOTES_DIR` to the local copy of that folder (Google Drive for Desktop). | Keep one GoodNotes folder per class named after the course code, e.g. `CS 231N`. Write the date (`9/24`) at the top of each day's first page: that's how pages get matched to lectures. Once Canvas is connected, notebooks for classes you're not taking this term are left out. New pages are transcribed by Claude after each sync (math as LaTeX), since GoodNotes' own text garbles math; Settings → GoodNotes turns that off. |
...
| **Granola** | `GRANOLA_API_KEY` from Granola → Settings → Connectors → API keys (Business plan), or, on any plan, run `studyhub granola login` and approve StudyHub in the browser | Record each lecture into a folder named after the course code. With the API, transcripts carry exact times; through the sign-in, times are estimated from where a passage falls in the transcript, and citations show them as “≈41:12”. StudyHub keeps its own copy of transcripts, so Granola's auto-deletion can't take them away. |
